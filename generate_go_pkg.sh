...
if [ "$#" -ne 4 ]; then
  ...
  echo "Example: "
  echo ""
  echo "  $0 contracts LiquidityMiningCampaign staker staker"
  echo ""
  echo "will create: ./go-staker/staker/LiquidityMiningCampaign.go"
  echo "based on: ./contracts/LiquidityMiningCampaign.sol"
  echo ""
  echo "  $0 contracts LockScheme lock staker/lock"
  echo ""
  echo "will create: ./go-staker/staker/lock/LockScheme.go"
  echo "based on: ./contracts/LockScheme.sol"
  echo "==========================================================="
  exit 1
fi
...
docker run --rm \
  ...
  openzeppelin-solidity/=/root/node_modules/openzeppelin-solidity/ --abi /root/${CONTRACT_PATH}/${CONTRACT_NAME}.sol --allow-paths '' -o /build
echo
