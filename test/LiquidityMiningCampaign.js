...
describe('LMC', () => {
  ...
  const setupRewardsPoolParametersNoLock = async (deployer) => {
    rewardTokensInstances = [];
    rewardTokensAddresses = [];
    rewardPerBlock = [];
    lockSchemеs = [];
  ...
  describe("No lock", () => {
    beforeEach(async () => {
      ...
      LmcInstanceNoLock = await deployer.deploy(
        LMC,
        {},
        stakingTokenAddress,
        startBlock,
    ...
    describe("Only Staking", () => {
      ...
      it("Should stake only sucessfully", async () => {
        let currentBlock = await deployer.provider.getBlock('latest');
        let contractInitialBalance = await stakingTokenInstance.balanceOf(LmcInstanceNoLock.contractAddress);
        let userInitialBalance = await stakingTokenInstance.balanceOf(aliceAccount.signer.address);

        await LmcInstanceNoLock.stakeAndLock(bTen, LockSchemeInstance0.contractAddress);

        let contractFinalBalance = await stakingTokenInstance.balanceOf(LmcInstanceNoLock.contractAddress);
        let userFinalBalance = await stakingTokenInstance.balanceOf(aliceAccount.signer.address);
        const totalStakedAmount = await LmcInstanceNoLock.totalStaked();
        const userInfo = await LmcInstanceNoLock.userInfo(aliceAccount.signer.address)
        const userRewardDebt = await LmcInstanceNoLock.getUserRewardDebt(aliceAccount.signer.address, 0);
        const userOwedToken = await LmcInstanceNoLock.getUserOwedTokens(aliceAccount.signer.address, 0);
        ...
        const accumulatedReward = await LmcInstanceNoLock.getUserAccumulatedReward(aliceAccount.signer.address, 0);
        assert(accumulatedReward.eq(bOne), "The reward accrued was not 1 token");
      })

      it("Should fail staking with zero amount", async () => {
        await assert.revertWith(LmcInstanceNoLock.stakeAndLock(0, LockSchemeInstance0.contractAddress), "stakeAndLock::Cannot stake 0");
      })
    })

    describe("Withdraw and Exit with no lock", () => {
      ...
      it("Should withdraw and exit sucessfully", async () => {
        ...
        const userInitialBalanceStaking = await stakingTokenInstance.balanceOf(aliceAccount.signer.address);
        const userInfoInitial = await LmcInstanceNoLock.userInfo(aliceAccount.signer.address);
        const userTokensOwedInitial = await LmcInstanceNoLock.getUserAccumulatedReward(aliceAccount.signer.address, 0);
        const initialTotalStakedAmount = await LmcInstanceNoLock.totalStaked();
        const userInitialBalanceRewards = await rewardTokensInstances[0].balanceOf(aliceAccount.signer.address);
        ...
        const userTokensOwed = await LmcInstanceNoLock.getUserOwedTokens(aliceAccount.signer.address, 0);
        const userFinalBalanceStaking = await stakingTokenInstance.balanceOf(aliceAccount.signer.address);
      ...
      xit("Should withdraw sucessfully when staked in two different lmcs", async () => {
        const { formatEther } = ethers.utils;
        ...
        let userInfoLock3 = await LockSchemeInstance03.userInfo(aliceAccount.signer.address);
        let userInfoLock6 = await LockSchemeInstance06.userInfo(aliceAccount.signer.address);
        ...
        assert(contractFinalBalance.eq(contractInitialBalance.sub(bTen).sub(bTwenty)), "The balance of the contract was not incremented properly")
        assert(userInfoLock3.balance.eq(bTwenty), "The transferred amount is not corrent");
        assert(userInfoLock6.balance.eq(0), "The transferred amount is not corrent in the second lock scheme");
        assert(userFinalBalanceRewards.eq(userInitialBalanceRewards.add(bonus6).add(bonus3).add(userTokensOwedInitial)), "The rewards balance is not correct")
        assert(totalStakedAmount.eq(bTen.add(bTwenty)), "The stake was not successful")
        assert(userInfo.amountStaked.eq(bTen.add(bTwenty)), "User's staked amount is not correct")
        assert(userAccruedRewards.eq(0), "User's accrued rewards should be zero")
      ...
      xit("Should withdraw sucessfully when staked in two different lmcs and called only exit", async () => {
      ...
      it("Should exit and stake sucessfully", async () => {
        ...
        let NewLmcInstanceNoLock = await deployer.deploy(
          LMC,
          {},
          stakingTokenAddress,
          startBlock,
        ...
        let lockScheme = [];
        LockSchemeInstance0 = await deployer.deploy(LockScheme, libraries, lockBlock0, rampUpBlock0, 0, NewLmcInstanceNoLock.contractAddress);
        lockScheme.push(LockSchemeInstance0.contractAddress);

        await NewLmcInstanceNoLock.setLockSchemes(lockScheme);
        await rewardTokensInstances[0].mint(NewLmcInstanceNoLock.contractAddress, amount);
        let externalRewardsTokenInstance = await deployer.deploy(TestERC20, {}, amount);
        await externalRewardsTokenInstance.mint(treasury.signer.address, amount);
        ...
        await stakingTokenInstance.approve(LockSchemeInstance0.contractAddress, amount);
        await stakingTokenInstance.approve(NewLmcInstanceNoLock.contractAddress, amount);

        await NewLmcInstanceNoLock.stakeAndLock(bTen, LockSchemeInstance0.contractAddress);
        await NewLmcInstanceNoLock.setReceiverWhitelisted(NonCompoundingRewardsPoolInstance.contractAddress, true);
        ...
        let initialBalance = await NonCompoundingRewardsPoolInstance.balanceOf(aliceAccount.signer.address);
        const userTokensOwedInitial = await NewLmcInstanceNoLock.getUserAccumulatedReward(aliceAccount.signer.address, 0);
        // const userInitialBalanceRewards = await rewardTokensInstances[1].balanceOf(aliceAccount.signer.address);

        //the bonus is 10% of the reward
        const bonus = 0;
        await NewLmcInstanceNoLock.exitAndStake(NonCompoundingRewardsPoolInstance.contractAddress);
        // const userFinalBalanceRewards = await rewardTokensInstances[1].balanceOf(aliceAccount.signer.address);

        let finalBalance = await NonCompoundingRewardsPoolInstance.balanceOf(aliceAccount.signer.address);
        let totalStakedAmount = await NonCompoundingRewardsPoolInstance.totalStaked()
        let userInfo = await NonCompoundingRewardsPoolInstance.userInfo(aliceAccount.signer.address)
        const userAccruedRewards = await NewLmcInstanceNoLock.userAccruedRewards(aliceAccount.signer.address);
        assert(finalBalance.gt(initialBalance), "Staked amount is not correct");
        assert(finalBalance.eq(userTokensOwedInitial.add(bonus)), "User rewards were not calculated properly");
        assert(totalStakedAmount.eq(userTokensOwedInitial.add(bonus)), "Total Staked amount is not correct");
        assert(userInfo.amountStaked.eq(finalBalance), "User's staked amount is not correct");
      ...
      it("Should fail calling the claim function only", async () => {
        await assert.revertWith(LmcInstanceNoLock.claim(), "OnlyExitFeature::cannot claim from this contract. Only exit.");
      })

      it("Should return from exit if the user hasn;t locked", async () => {
        await LmcInstanceNoLock.from(bobAccount.signer.address).exitAndUnlock();
      })

      it("Should return from exit fomr the exit and stake if the user hasn't locked", async () => {
        await LmcInstanceNoLock.setReceiverWhitelisted(aliceAccount.signer.address, true);
        await LmcInstanceNoLock.from(bobAccount.signer.address).exitAndStake(aliceAccount.signer.address);
      })

      it("Should fail to exit and stake if the poolAddress is not whitelisted ", async () => {
        await assert.revertWith(LmcInstanceNoLock.exitAndStake(aliceAccount.signer.address), "exitAndTransfer::receiver is not whitelisted");
      })
    })
  })
});