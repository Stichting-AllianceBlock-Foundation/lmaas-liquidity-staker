...
//   // console.log('\x1b[36m%s\x1b[0m', `--- Treasury address: ${TreasuryInstance.contractAddress} ---`)
  ...
  const mint1 = await albtInstance.mint(CompoundingRewardsPoolFactoryInstance.contractAddress, allRewards, gasPrice)
  const mintStatus1 = await mint1.wait()
  const mint2 = await albtInstance.mint(CompoundingRewardsPoolFactoryInstance.contractAddress, allRewards, gasPrice)
  const mintStatus2 = await mint2.wait()
  const mint3 = await albtInstance.mint(CompoundingRewardsPoolFactoryInstance.contractAddress, allRewards, gasPrice)
  ...
  console.log('\x1b[36m%s\x1b[0m', `--- First NonCompoundingPool address 0: ${nonCompoundingPool1} ---`)
  let nonCompoundingPool2 = await CompoundingRewardsPoolFactoryInstance.rewardsPools(2)
  console.log('\x1b[36m%s\x1b[0m', `--- First NonCompoundingPool address 0: ${nonCompoundingPool2} ---`)
  let nonCompoundingPool3 = await CompoundingRewardsPoolFactoryInstance.rewardsPools(3)
  console.log('\x1b[36m%s\x1b[0m', `--- First NonCompoundingPool address 0: ${nonCompoundingPool3} ---`)
...
stakerContracts: {
  ...
  let enable = await CompoundingRewardsPoolFactoryInstance.enableReceivers(nonCompoundingPool0, [nonCompoundingPool2,nonCompoundingPool3])
  enable.wait()
  let enable1 = await CompoundingRewardsPoolFactoryInstance.enableReceivers(nonCompoundingPool1, [nonCompoundingPool2,nonCompoundingPool3])
  console.log(enable)
  console.log(enable1)
}
