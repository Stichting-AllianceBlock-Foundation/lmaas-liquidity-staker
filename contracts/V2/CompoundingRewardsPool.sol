...
contract CompoundingRewardsPool is RewardsPoolBase, OneStakerFeature {
	...
	) public RewardsPoolBase(_stakingToken, _startBlock, _endBlock, _rewardsTokens, _rewardPerBlock, _stakeLimit, _contractStakeLimit) OneStakerFeature(_staker) {
	...
	function addMoreRewards(address rewardsToken, uint256 _tokenAmount) public {
		amountTransferred += _tokenAmount;
		IERC20Detailed(rewardsToken).safeTransfer(address(this), _tokenAmount);
