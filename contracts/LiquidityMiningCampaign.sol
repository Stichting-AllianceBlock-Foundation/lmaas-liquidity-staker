...
contract LiquidityMiningCampaign is StakeTransferer, OnlyExitFeature {
    ...
    function setReceiverWhitelisted(address receiver, bool whitelisted) public override(StakeTransferer) {
    ...
    function setLockSchemes(address[] calldata _lockSchemes) external {
        ...
        for (uint256 i = 0; i < _lockSchemes.length; i++) {
            address resultLockScheme = _lockSchemes[i];

            if (!lockSchemesExist[resultLockScheme]) {
                lockSchemes.push(resultLockScheme);
                lockSchemesExist[resultLockScheme] = true;
            }
        }
    }
}
